...
const SavedJobs = () => {
  ...
  useEffect(() => {
    const fetchSavedJobs = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/applicant/savedJobs/${userId}`);
        setAdminJobs(response.data); // Backend returns array directly
        if (response.data.length > 0) {
          setSelectedJobId(response.data[0].id); // Auto-select first job
        }
      } catch (error) {
        toast.error("Can't fetch saved jobs");
      }
    };
    fetchSavedJobs();
  }, [userId]);
  ...
  const JobList = ({ jobs, selectedJobId, onJobSelect }) => (
    <div className="w-full lg:w-2/5 xl:w-1/3 space-y-4 pr-4 lg:pr-0 overflow-y-auto max-h-[calc(100vh-200px)]">
      ...
      <p className="text-sm text-gray-600 mb-4">Your saved jobs list</p>
      {jobs.length > 0 ? (
        jobs.map(job => (
          <JobCard key={job.id} job={job} isSelected={job.id === selectedJobId} onSelect={onJobSelect} />
        ))
      ) : (
        <p className="text-gray-500 text-sm">No saved jobs found.</p>
      )}
    </div>
  );
  ...
  const JobDetails = ({ job }) => {
    ...
    return (
      <div className="w-full lg:w-3/5 xl:w-2/3 bg-white p-8 rounded-xl shadow-md overflow-y-auto max-h-[calc(100vh-200px)]">
        ...
        <button id="appbtn" className="w-30 bg-blue-400 h-10 rounded font cursor-pointer">Apply Now</button>
  ...
  return (
    <div className="flex h-screen bg-gray-100 font-inter relative">
      ...
      <div className="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8 font-inter flex-1 overflow-y-auto">
        <SearchBar />
        <center><h1 className='p-10 text-2xl'>Saved Jobs</h1></center>
